...
def create_or_update_profile(request):
    try:
        ...
        user_profile, created = UserProfile.objects.update_or_create(
            uid=uid,
            defaults={'email': email, 'name': data.get('name', ''), 'phone': data.get('phone', ''), 'about': data.get('about', ''), 'approvalStatus': data.get('approvalStatus', ''), 'approvedAgent': data.get('approvedAgent', ''), 'verified': data.get('verified', '')}
        )
...
@api_view(['POST'])
@csrf_exempt
def add_or_update_hostels(request):
    try:
        data = request.data
        token = data.get('token')
        if not token:
        ...
        hostel, created = AgentListing.objects.update_or_create(
            uid=uid,
            defaults={
        ...
        # Serialize the hostel listing
        serializer = AgentListingSerializer(hostel)  # Assuming you have a HostelSerializer
        if created:
            return Response({'message': 'Hostel created', 'hostel': serializer.data}, status=status.HTTP_201_CREATED)
        else:
            return Response({'message': 'Hostel updated', 'hostel': serializer.data}, status=status.HTTP_200_OK)
...
class AgentListingCreateView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            agent_listing = serializer.save(owner=user_profile)
